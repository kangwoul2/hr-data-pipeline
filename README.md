...
IBM HR 데이터를 단순 분석 노트북에 머무르게 하지 않고, **데이터 품질 검사 → 변환 → PostgreSQL 적재 → Airflow 실행 관리 → FastAPI 조회 제공**까지 확장한 데이터 엔지니어링 프로젝트입니다.
...
FastAPI는 분석을 새로 수행하는 역할이 아니라 **정제·집계된 결과를 다른 시스템이 조회할 수 있게 제공하는 역할**입니다.
...
> 기존에는 노트북에서 HR 데이터를 분석했지만 실행 순서와 데이터 상태에 따라 결과가 달라질 수 있었습니다. 그래서 필수 열, 중복 직원, 잘못된 값과 누락값을 적재 전에 검사하는 데이터 품질 단계를 만들고, 검사를 통과한 데이터만 PostgreSQL에 넣도록 했습니다. `to_sql(replace)`처럼 테이블을 다시 만드는 방식은 DB 제약조건을 잃을 수 있어서 스키마는 유지하고 하나의 트랜잭션 안에서 데이터를 다시 적재합니다. Airflow는 이 과정을 정해진 순서로 반복 실행하기 위해 사용했고, FastAPI는 만들어진 집계 결과를 다른 서비스가 조회할 수 있도록 분리했습니다.

### 자주 나오는 질문

- 왜 잘못된 행을 버리지 않고 전체 작업을 실패시키는가?
- 왜 `to_sql(replace)`를 사용하지 않았는가?
- 애플리케이션 검사와 DB 제약조건을 둘 다 쓰는 이유는?
- Airflow와 Kafka는 어떤 문제를 각각 해결하는가?
- ETL 적재를 왜 하나의 트랜잭션으로 묶는가?
- 부서별 집계 테이블을 미리 만드는 이유는?
- 인덱스가 실제로 필요한지 어떻게 확인하는가?
- 데이터가 커지면 전체 재적재를 어떻게 바꿀 것인가?
...
이 프로젝트는 **데이터 품질 → 변환 → 트랜잭션 적재 → 실행 관리 → 조회 제공** 순서로 설명합니다.